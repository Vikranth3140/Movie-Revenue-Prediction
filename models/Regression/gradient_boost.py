...
plt.scatter(np.exp(y_train) - 1, np.exp(train_predictions) - 1, color='blue', alpha=0.5, label=f'Train (R² = {train_r2:.4f})')
plt.scatter(np.exp(y_test) - 1, np.exp(test_predictions) - 1, color='red', alpha=0.5, label=f'Test (R² = {test_r2:.4f})')
