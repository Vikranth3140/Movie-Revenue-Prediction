...
def check_duplicates1(df1, df2):
    df2.rename(columns={'Movie_Title': 'name'}, inplace=True)      #for initial_merge
    # Concatenate the two dataframes along the row axis
    df = pd.concat([df1, df2], axis=0)
    
    # Find the rows that are duplicated in the concatenated dataframe
    duplicated = df.duplicated(subset='name', keep=False)
    
    
    # Count the number of True values in the duplicated series
    ...
    # Return the dataframe with unique movies
    return df
def check_duplicates2(df1, df2):
    df1.rename(columns={'title': 'name'}, inplace=True)  
    df2.rename(columns={'movie_title': 'name'}, inplace=True)  #For intermediate merge
    # Concatenate the two dataframes along the row axis
    df = pd.concat([df1, df2], axis=0)
